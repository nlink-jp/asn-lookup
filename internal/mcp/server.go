...
// the standard library, exposing asn-lookup's database as MCP tools.
package mcp
...
// calls do not reparse the index, while still picking up update_db changes.
type server struct {
	e       *engine.Engine
	version string
...
func (s *server) initializeResult(params json.RawMessage) any {
	...
	return map[string]any{
		...
		"serverInfo":      map[string]any{"name": "asn-lookup", "version": s.version},
		"instructions":    Instructions,
	}
}

// database returns the cached index, reloading it when the file changed on disk.
func (s *server) database() (*asndb.DB, error) {
	fi, err := os.Stat(s.e.Cfg.DBPath)
	if err != nil {
