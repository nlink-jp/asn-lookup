...
名前を挙げて失敗します（`arguments: json: unknown field "offest"`）。従来は無視
して実行していたため、`offset` の綴り間違いは大きな AS のどのページ要求も先頭
ページとして返していました。型が違う引数も同様に拒否されます。引数のデコードより
前には何も実行しません。引数をまったく渡さない呼び出しは従来どおり「引数なし」
として扱われます。
